...
Because the AIC for model with stratum-specific detection functions (`r round(model.separate.AIC)`) is less than AIC for model with pooled detection function (`r round(model.pooled.AIC$AIC)`), we base our inference upon the stratum-specific detection function model.
...
Further exploration of analyses involving stratification can be found in the [example of dung survey analysis](https://examples.distancesampling.org/Distance-dung/dung.html).
